...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="warehouse_management_user" lay-filter="warehouse_management_user"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/warehouse_management_user/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/warehouse_management_user/table', 'del') || $check_action('/warehouse_management_user/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/warehouse_management_user/table', 'add') || $check_action('/warehouse_management_user/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/warehouse_management_user/view', 'get') || $check_action('/warehouse_management_user/view', 'set')) {
            detail_flag = true;
        }
        ...
        function getpath() {
            ...
            for (var i = 0; i < list_data.length; i++) {
                var o = list_data[i];
                if (o.path === "/warehouse_management_user/table") {
                    console.log(o.path);
                    path1 = o.path
                        $get_power(o.path)
                }
        ...
        , cols: [[
            ...
            field: 'user_id', width: 200, title: '用户名', templet: function (d) {
                ...
                field: 'nickname', width: 150, title: '昵称', templet: function (d) {
                    ...
                    , {
                field: 'warehouse_management_personnel', width: 180, title: '仓管人员', sort: true
    ...
    ,
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                                    ...
                                    if (user_group === '管理员' || $check_field('get', 'warehouse_management_personnel', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='warehouse_management_personnel']").css('display', 'none');
                    }
    ...
    ,
        parseData: function (res) {
            ...
            if (user_group == "管理员" || $check_pay('/warehouse_management_user/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/warehouse_management_user/view', 'set') || $check_action('/warehouse_management_user/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(warehouse_management_user)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('warehouse_management_user', BaseUrl + '/api/warehouse_management_user/del', 'warehouse_management_user_id', request, resetName, resetSelect                        , undefined, undefined,true
        )
