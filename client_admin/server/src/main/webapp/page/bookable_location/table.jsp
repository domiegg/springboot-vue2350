...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="bookable_location" lay-filter="bookable_location"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/bookable_location/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/bookable_location/table', 'del') || $check_action('/bookable_location/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/bookable_location/table', 'add') || $check_action('/bookable_location/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/bookable_location/view', 'get') || $check_action('/bookable_location/view', 'set')) {
            detail_flag = true;
        }
        ...
        function getpath() {
            ...
            for (var i = 0; i < list_data.length; i++) {
                var o = list_data[i];
                if (o.path === "/bookable_location/table") {
                    console.log(o.path);
                    path1 = o.path
                        $get_power(o.path)
                }
    ...
    ,
        parseData: function (res) {
            ...
            if (user_group == "管理员" || $check_pay('/bookable_location/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/bookable_location/view', 'set') || $check_action('/bookable_location/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(bookable_location)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.bookable_location_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('bookable_location', BaseUrl + '/api/bookable_location/del', 'bookable_location_id', request, resetName, resetSelect        )
