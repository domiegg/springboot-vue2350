...
<script>
    var BaseUrl = baseUrl()
    let bookable_location_id = location.search.substring(1)
    layui.use(['upload', 'element', 'layer', 'laydate', 'layedit'], function () {
        ...
        // 提交按钮校验权限
        // if (   user_group == "管理员" ||$check_action('/bookable_location/view', 'add') || $check_action('/bookable_location/view', 'set')) {
        //    submit.style.display = "block"
        // }
        // style="display: none"

        
            let field = "bookable_location_id";
            let url_add = "bookable_location";
            let url_set = "bookable_location";
            let url_get_obj = "bookable_location";
            let url_upload = "bookable_location"
            let query = {
                "bookable_location_id": 0,
            }

            let form_data2 = {
                                        "bookable_location":  '', // 可预约地点
                                    "bookable_location_id": 0, // ID
                                            }

            layui.layedit.set({
      uploadImage: {
        url: BaseUrl + '/api/bookable_location/upload?' //接口url
        , type: 'post' //默认post
      }
    });
            ...
            function $check_field(action, field) {
                var o = $get_power("/bookable_location/view");
                var auth;
                if (o && o[action] != 0 && o[action] != false) {
                    auth = o["field_" + action];
                }
            ...
            if (bookable_location_id !== '') {
                async function axios_get_3() {
                    const {data: rese} = await axios.get(BaseUrl + '/api/bookable_location/get_obj', {
                        params: {
                                bookable_location_id: bookable_location_id
                        }, headers: {
                            'x-auth-token': token
                        }
                    })
                    ...
                    // Array.prototype.slice.call(document.getElementsByTagName('textarea')).map(i => i.disabled = true)
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['bookable_location_id'] && $check_field('set', 'bookable_location')) || (!form_data2['bookable_location_id'] && $check_field('add', 'bookable_location'))) {
                            }else {
            ...
            submit.onclick = async function () {
                ...
                if (bookable_location_id == '') {
                                        console.log("新增/Add")
                    const {data: res} = await axios.post(BaseUrl + '/api/bookable_location/add?',
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
              ...
              layer.msg(res.error.message);
                    ...
                    const {data: res} = await axios.post(BaseUrl + '/api/bookable_location/set?bookable_location_id=' + bookable_location_id,
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
                                    'Content-Type': 'application/json'
