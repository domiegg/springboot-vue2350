...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="warehousing_records" lay-filter="warehousing_records"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/warehousing_records/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/warehousing_records/table', 'del') || $check_action('/warehousing_records/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/warehousing_records/table', 'add') || $check_action('/warehousing_records/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/warehousing_records/view', 'get') || $check_action('/warehousing_records/view', 'set')) {
            detail_flag = true;
        }
    ...
    table.render({
        ...
        , cols: [[
                ...
                field: 'expiration_date', width: 180, title: '失效日期', sort: true
                                ...
                                , {
                field: 'receipt_quantity', width: 180, title: '入库数量', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='expiration_date']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'receipt_quantity', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='receipt_quantity']").css('display', 'none');
                    }
    ...
    ,
        parseData: function (res) {
            ...
            if (user_group == "管理员" || $check_pay('/warehousing_records/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/warehousing_records/view', 'set') || $check_action('/warehousing_records/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(warehousing_records)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.warehousing_records_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('warehousing_records', BaseUrl + '/api/warehousing_records/del', 'warehousing_records_id', request, resetName, resetSelect        )
