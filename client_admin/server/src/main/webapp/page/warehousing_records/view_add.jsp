                                            ...
                                            <div class="layui-input-block">
                                                <input type="text" name="title" lay-verify="title" autocomplete="off"
                                                       ...
                                                       class="layui-input" id="expiration_date">
                                                                                                                    ...
                                                                                                                    <div class="layui-form-item" id="receipt_quantity_box">
                                        <label class="layui-form-label">入库数量</label>
                                        <div class="layui-input-block">
                                            <input type="number" name="num" class="layui-input num" id="receipt_quantity">
                                        </div>
                                    </div>
...
<script>
    var BaseUrl = baseUrl()
    let warehousing_records_id = location.search.substring(1)
    layui.use(['upload', 'element', 'layer', 'laydate', 'layedit'], function () {
        ...
        // 提交按钮校验权限
        // if (   user_group == "管理员" ||$check_action('/warehousing_records/view', 'add') || $check_action('/warehousing_records/view', 'set')) {
        //    submit.style.display = "block"
        // }
        // style="display: none"

        
            let field = "warehousing_records_id";
            let url_add = "warehousing_records";
            let url_set = "warehousing_records";
            let url_get_obj = "warehousing_records";
            let url_upload = "warehousing_records"
            let query = {
                "warehousing_records_id": 0,
            }
            ...
            let form_data2 = {
                            ...
                            "receipt_quantity":  0, // 入库数量
                                    "warehousing_records_id": 0, // ID
                                            }

            layui.layedit.set({
      uploadImage: {
        url: BaseUrl + '/api/warehousing_records/upload?' //接口url
        , type: 'post' //默认post
      }
    });
            ...
            function getpath() {
                ...
                for (var i = 0; i < list_data.length; i++) {
                    var o = list_data[i];
                    if (o.path === "/warehousing_records/table") {
                        path1 = o.path
                            $get_power(o.path)
                    }
                }
            ...
            function $check_field(action, field) {
                var o = $get_power("/warehousing_records/view");
                var auth;
                if (o && o[action] != 0 && o[action] != false) {
                    auth = o["field_" + action];
                }
                ...
                return false;
                    ...
                    if ($check_field('add', 'expiration_date')){
                        ...
                        $("#expiration_date_box").hide()
                            ...
                            if (user_group === '管理员') {
                    $("#receipt_quantity_box").show()
                } else {
                    if ($check_field('add', 'receipt_quantity')){
                        $("#receipt_quantity_box").show()
                    }else {
                        $("#receipt_quantity_box").hide()
                    }
                }
                    ...
                    //文本
                                                                                                //数字
                    let receipt_quantity = document.querySelector("#receipt_quantity")
                        receipt_quantity.onkeyup = function (event) {
                        form_data2.receipt_quantity = Number(event.target.value)
                    }
                    //数字
            ...
            if (warehousing_records_id !== '') {
                async function axios_get_3() {
                    const {data: rese} = await axios.get(BaseUrl + '/api/warehousing_records/get_obj', {
                        params: {
                                warehousing_records_id: warehousing_records_id
                        }, headers: {
                            'x-auth-token': token
                        }
                    })
                    ...
                    Object.keys(form_data2).forEach((key) => {
                        ...
                        $("#"+key).val(form_data2[key])
                            ...
                            if ($check_field('set', 'receipt_quantity') || $check_field('get', 'receipt_quantity')){
                                $("#receipt_quantity_box").show()
                            }else {
                                $("#receipt_quantity_box").hide()
                            }
                    ...
                    // Array.prototype.slice.call(document.getElementsByTagName('textarea')).map(i => i.disabled = true)
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['warehousing_records_id'] && $check_field('set', 'vaccine_no')) || (!form_data2['warehousing_records_id'] && $check_field('add', 'vaccine_no'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['warehousing_records_id'] && $check_field('set', 'vaccine_name')) || (!form_data2['warehousing_records_id'] && $check_field('add', 'vaccine_name'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['warehousing_records_id'] && $check_field('set', 'vaccine_classification')) || (!form_data2['warehousing_records_id'] && $check_field('add', 'vaccine_classification'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['warehousing_records_id'] && $check_field('set', 'suitable_for_people')) || (!form_data2['warehousing_records_id'] && $check_field('add', 'suitable_for_people'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['warehousing_records_id'] && $check_field('set', 'manufacturer')) || (!form_data2['warehousing_records_id'] && $check_field('add', 'manufacturer'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['warehousing_records_id'] && $check_field('set', 'date_of_manufacture')) || (!form_data2['warehousing_records_id'] && $check_field('add', 'date_of_manufacture'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['warehousing_records_id'] && $check_field('set', 'expiration_date')) || (!form_data2['warehousing_records_id'] && $check_field('add', 'expiration_date'))) {
                            }else {
                                $("#expiration_date").attr("disabled", true);
                                $("#expiration_date > input[name='file']").attr('disabled', true);
                            }
                                                            //数字
                                receipt_quantity.value = form_data2.receipt_quantity
                            //数字
                            
                                if (user_group === '管理员' || (form_data2['warehousing_records_id'] && $check_field('set', 'receipt_quantity')) || (!form_data2['warehousing_records_id'] && $check_field('add', 'receipt_quantity'))) {
                            }else {
                                $("#receipt_quantity").attr("disabled", true);
                                $("#receipt_quantity > input[name='file']").attr('disabled', true);
                            }
            ...
            submit.onclick = async function () {
                try {
                            ...
                            //文本
                                                                    //数字
                            form_data2.receipt_quantity = Number(receipt_quantity.value)
                            //数字
                                        } catch (err) {
                    console.log(err)
                }
                ...
                let customize_field = []
                                            customize_field.push({"field_name": "疫苗编号", "field_value": form_data2.vaccine_no});
                                                ...
                                                customize_field.push({"field_name": "入库数量", "field_value": form_data2.receipt_quantity});
                    
    
                if (warehousing_records_id == '') {
                                        console.log("新增/Add")
                    const {data: res} = await axios.post(BaseUrl + '/api/warehousing_records/add?',
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
              ...
              layer.msg(res.error.message);
                    ...
                    const {data: res} = await axios.post(BaseUrl + '/api/warehousing_records/set?warehousing_records_id=' + warehousing_records_id,
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
                                    'Content-Type': 'application/json'
