        ...
        <div class="layui-container">
            <div class="layui-row">
                <form class="layui-form" action="">
                            ...
                            <button type="button" class="layui-btn" id="vaccination_personnel">上传头像</button>
                            <div class="layui-upload-list">
                                <img class="layui-upload-img" id="vaccination_personnel_img">
                                <p id="demoText"></p>
                            </div>
                            <div style="width: 95px;">
                                <div class="layui-progress layui-progress-big" lay-showpercent="yes"
                                     lay-filter="vaccination_personnel">
                            ...
                            <div class="layui-input-block">
                                <select name="interest" lay-filter="state" id="state">
                                    ...
                                    <option value="4">已注销</option>
                                            ...
                                            <div class="layui-input-block">
                                                <input type="text" name="title" lay-verify="title" autocomplete="off"
                                                       placeholder="请输入接种工作人员"
                                                       class="layui-input" id="vaccination_staff">
                                            </div>
                                        </div>
...
<script>
    var BaseUrl = baseUrl()
    let vaccination_personnel_id = location.search.substring(1)
    layui.use(['upload', 'element', 'layer', 'laydate', 'layedit'], function () {
        ...
        // 提交按钮校验权限
        // if (   user_group == "管理员" ||$check_action('/vaccination_personnel/view', 'add') || $check_action('/vaccination_personnel/view', 'set')) {
        ...
        // style="display: none"
            ...
            var uploadInst = upload.render({
                elem: '#vaccination_personnel'
                , url: BaseUrl + '/api/vaccination_personnel/upload?' //此处用的是第三方的 http 请求演示，实际使用时改成您自己的上传接口即可。
                , headers: {
                    'x-auth-token': token
                }, before: function (obj) {
                    //预读本地文件示例，不支持ie8
                    obj.preview(function (index, file, result) {
                        $('#vaccination_personnel_img').attr('src', fullUrl(BaseUrl,result)); //图片链接（base64）
                    });

                    element.progress('vaccination_personnel', '0%'); //进度条复位
                ...
                , progress: function (n, elem, e) {
                    element.progress('vaccination_personnel', n + '%'); //可配合 layui 进度条元素使用
                    if (n == 100) {
            ...
            let form_sub = {
                                        "vaccination_staff":  '', // 接种工作人员
    
                            "user_id": 0,
                "vaccination_personnel_id": 0 // ID
            }
            ...
            layui.form.on('select(phone_state)', function (data) {
                ...
                console.log()
                                                                                                    ...
                                                                                                    let vaccination_staff = document.querySelector("#vaccination_staff")
                                                
            if (vaccination_personnel_id !== '') {
                ...
                async function axios_get_4() {
                    const {data: rese} = await axios.get(BaseUrl + '/api/user/get_obj', {
                        params: {
                            user_id: vaccination_personnel_id
                        }
                    })
                    ...
                    for (let key in form_data) {
                        if (key == 'avatar') {
                                vaccination_personnel_img.src = BaseUrl + form_data.avatar
                        }
                    }
                    for (let key in form_data) {
                        if (key == 'email_state') {
                ...
                async function axios_get_5() {
                    const {data: rese} = await axios.get(BaseUrl + '/api/vaccination_personnel/get_obj', {
                        params: {
                            user_id: vaccination_personnel_id
                        }
                    })
                   ...
                   for (let key in form_sub) {
                        ...
                        vaccination_staff.value = form_sub.vaccination_staff
                                                            }
            ...
            submit.onclick = async function () {
                ...
                form_data.avatar = url
                                                                                                    //7
                                form_sub.vaccination_staff = vaccination_staff.value
                                //7
                ...
                if (!username.value) {
                    ...
                    layer.msg('密码不能为空');
                } else if (!vaccination_personnel_id && (password.value.length > 16 || password.value.length < 5)) {
                    ...
                    if (vaccination_personnel_id !== '') {
                        data_p['user_id'] = vaccination_personnel_id
                    }
                    let data_p_param = {
                    ...
                    form_sub.user_id = idd
                            ...
                            form_sub.vaccination_staff = vaccination_staff.value
                            //多文本
                                                            if (user_group === '管理员' || (form_sub['vaccination_personnel_id'] && $check_field('set', 'vaccination_staff')) || (!form_sub['vaccination_personnel_id'] && $check_field('add', 'vaccination_staff'))) {
                            }else {
                                ...
                                $("#vaccination_staff > input[name='file']").attr('disabled', true);
                                                ...
                                                if (vaccination_personnel_id == '') {
                        console.log("新增/Add")
                        const {data: ress} = await axios.post(BaseUrl + '/api/vaccination_personnel/add?', form_sub, {
                            headers: {
                                'x-auth-token': token,
                                'Content-Type': 'application/json'
                            }
                        ...
                        const {data: res_data2} = await axios.post(BaseUrl + '/api/vaccination_personnel/set?vaccination_personnel_id=' + form_sub.vaccination_personnel_id, form_sub, {
                            headers: {
                                'x-auth-token': token,
                                'Content-Type': 'application/json'
                            }
