            ...
            <div class="layui-row">
                <form class="layui-form" action="">
                                            ...
                                            <div class="layui-input-block">
                                                ...
                                                <div class="layui-input-block">
                                                    <select name="interest" lay-filter="vaccination_staff"
                                                            id="vaccination_staff">
                                                        <option value=""></option>
                                                    </select>
                                                </div>
                                            </div>
                                                                                                                                        <div class="layui-form-item layui-form-text" id="matters_needing_attention_box">
                                        <label class="layui-form-label">注意事项</label>
                                        <div class="layui-input-block">
                                            <textarea placeholder="请输入注意事项" class="layui-textarea"
                                                      id="matters_needing_attention"></textarea>
                                        </div>
                                    </div>
...
<script>
    var BaseUrl = baseUrl()
    let vaccination_records_id = location.search.substring(1)
    layui.use(['upload', 'element', 'layer', 'laydate', 'layedit'], function () {
        ...
        // 提交按钮校验权限
        // if (   user_group == "管理员" ||$check_action('/vaccination_records/view', 'add') || $check_action('/vaccination_records/view', 'set')) {
        //    submit.style.display = "block"
        // }
        // style="display: none"

        
            let field = "vaccination_records_id";
            let url_add = "vaccination_records";
            let url_set = "vaccination_records";
            let url_get_obj = "vaccination_records";
            let url_upload = "vaccination_records"
            let query = {
                "vaccination_records_id": 0,
            }
            ...
            let form_data2 = {
                            ...
                            "matters_needing_attention":  '', // 注意事项
                                    "vaccination_records_id": 0, // ID
                                            }

            layui.layedit.set({
      uploadImage: {
        url: BaseUrl + '/api/vaccination_records/upload?' //接口url
        , type: 'post' //默认post
      }
    });
            ...
            function getpath() {
                ...
                for (var i = 0; i < list_data.length; i++) {
                    var o = list_data[i];
                    if (o.path === "/vaccination_records/table") {
                        path1 = o.path
                            $get_power(o.path)
                    }
                }
            ...
            function $check_field(action, field) {
                var o = $get_power("/vaccination_records/view");
                var auth;
                if (o && o[action] != 0 && o[action] != false) {
                    auth = o["field_" + action];
                }
            ...
            function $check_examine() {
                ...
                return false;
                    ...
                    //文本
                                                                                                                                                    //多文本
                    let matters_needing_attention = document.querySelector("#matters_needing_attention")
                    //多文本
                                ...
                                for (let key in res.result.obj) {
                                    if (key == 'user_id') {
                                        ...
                                        for (let i = 0; i < alls.length; i++) {
                                            if (alls[i].value == test) {
                                                alls[i].selected = true
                                                $('#vaccination_staff').attr('disabled', 'disabled')
                                                form_data2.vaccination_staff = alls[i].value
                                                layui.form.render("select");
                                            }
                                        }
            ...
            if (vaccination_records_id !== '') {
                async function axios_get_3() {
                    const {data: rese} = await axios.get(BaseUrl + '/api/vaccination_records/get_obj', {
                        params: {
                                vaccination_records_id: vaccination_records_id
                        }, headers: {
                            'x-auth-token': token
                        }
                    })
                    ...
                    // Array.prototype.slice.call(document.getElementsByTagName('textarea')).map(i => i.disabled = true)
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'appointment_number')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'appointment_number'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'vaccine_no')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'vaccine_no'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'vaccine_name')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'vaccine_name'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'vaccine_classification')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'vaccine_classification'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'suitable_for_people')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'suitable_for_people'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'bookable_location')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'bookable_location'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'time_of_appointment')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'time_of_appointment'))) {
                            }else {
                                $("#time_of_appointment").attr("disabled", true);
                                $("#time_of_appointment > input[name='file']").attr('disabled', true);
                            }
                                    
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'user_name')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'user_name'))) {
                            }else {
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'telephone')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'telephone'))) {
                            }else {
                                $("#telephone").attr("disabled", true);
                                $("#telephone > input[name='file']").attr('disabled', true);
                            }
                                    
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'vaccination_staff')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'vaccination_staff'))) {
                            }else {
                                $("#vaccination_staff").attr("disabled", true);
                                $("#vaccination_staff > input[name='file']").attr('disabled', true);
                            }
                                                                //多文本
                                matters_needing_attention.value = form_data2.matters_needing_attention
                            //多文本
                        
                                if (user_group === '管理员' || (form_data2['vaccination_records_id'] && $check_field('set', 'matters_needing_attention')) || (!form_data2['vaccination_records_id'] && $check_field('add', 'matters_needing_attention'))) {
                            }else {
            ...
            submit.onclick = async function () {
                try {
                            ...
                            //文本
                                                                                                //多文本
                            form_data2.matters_needing_attention = matters_needing_attention.value
                            //多文本
                                            } catch (err) {
                    console.log(err)
                }
                ...
                if (vaccination_records_id == '') {
                                        console.log("新增/Add")
                    const {data: res} = await axios.post(BaseUrl + '/api/vaccination_records/add?',
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
              ...
              layer.msg(res.error.message);
                    ...
                    const {data: res} = await axios.post(BaseUrl + '/api/vaccination_records/set?vaccination_records_id=' + vaccination_records_id,
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
                                    'Content-Type': 'application/json'
