...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="vaccination_records" lay-filter="vaccination_records"></table>

        <script type="text/html" id="toolbarDemo">
            <div class="layui-btn-container">

                                            <!--跨表按钮-->
                                                    {{# if(d.add_vaccination_research>0){ }}
                            <button class="layui-btn layui-btn-sm" lay-event="vaccination_research">接种调研</button>
                            {{# } }}
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/vaccination_records/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/vaccination_records/table', 'del') || $check_action('/vaccination_records/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/vaccination_records/table', 'add') || $check_action('/vaccination_records/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/vaccination_records/view', 'get') || $check_action('/vaccination_records/view', 'set')) {
            detail_flag = true;
        }
        ...
        , cols: [[
                ...
                field: 'vaccination_staff', width: 180, title: '接种工作人员', sort: true
                            ...
                            function (d) {
                                ...
                                , {
                field: 'matters_needing_attention', width: 180, title: '注意事项', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='vaccination_staff']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'matters_needing_attention', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='matters_needing_attention']").css('display', 'none');
                    }
    ...
    ,
        parseData: function (res) {
                            ...
                            for (var i = 0; i < res.result.list.length; i++) {
                                res.result.list[i].add_vaccination_research = 1
                            }
                        }
                        
            if (user_group == "管理员" || $check_pay('/vaccination_records/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/vaccination_records/view', 'set') || $check_action('/vaccination_records/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(vaccination_records)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.vaccination_records_id;
                    }
            
                                else if (obj.event === 'vaccination_research'){
                        sessionStorage.setItem('data', JSON.stringify(data))
                        window.location.href = "../vaccination_research/view_add.jsp?"
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('vaccination_records', BaseUrl + '/api/vaccination_records/del', 'vaccination_records_id', request, resetName, resetSelect        )
